...
static void test_mem(size_t size_in) {
    DATA buffer[size_in];
    DATA *buffer_ptr = buffer;  //edit to test alignment
    ...
    // test write + read
    {
        ASSERT_EQ(DTU::write(0, msg_ptr, size_in * sizeof(DATA), 0, 0), Error::NONE);
        ASSERT_EQ(DTU::read(0, buffer_ptr, size_in * sizeof(DATA), 0, 0), Error::NONE);
        for(size_t i = 0; i < size_in; i++)
            ASSERT_EQ(buffer_ptr[i], msg_ptr[i]);
    }
}
...
static void test_msg(size_t msg_size_in, size_t reply_size_in) {
    const size_t TOTAL_MSG_SIZE = msg_size_in * sizeof(DATA) + sizeof(DTU::Header) + 8;
    const size_t TOTAL_REPLY_SIZE = reply_size_in * sizeof(DATA) + sizeof(DTU::Header) + 8;
    
    char rbuffer[2 * TOTAL_MSG_SIZE];
    char rbuffer2[2 * TOTAL_REPLY_SIZE];

    char *rbuffer_ptr = rbuffer;
    char *rbuffer2_ptr = rbuffer2;

    // prepare test data
    DATA msg[msg_size_in];
    DATA reply[reply_size_in];
    DATA *msg_ptr = msg;
    DATA *reply_ptr = reply;
    for(size_t i = 0; i < msg_size_in; ++i)
        msg_ptr[i] = i + 1;
    for(size_t i = 0; i < reply_size_in; ++i)
        reply_ptr[i] = reply_size_in - i;

    reg_t slot_msgsize = cLog2(TOTAL_MSG_SIZE) + 1;
    reg_t slot_replysize = cLog2(TOTAL_REPLY_SIZE) + 1;

    DTU::config_recv(1, reinterpret_cast<uintptr_t>(rbuffer_ptr), slot_msgsize+1, slot_msgsize, 3);
    DTU::config_recv(2, reinterpret_cast<uintptr_t>(rbuffer2_ptr), slot_replysize+1, slot_replysize, DTU::NO_REPLIES);

    // send + recv + reply
    {
        DTU::config_send(0, 0x1234, OWN_MODID, 1, slot_msgsize, 1);

        ASSERT_EQ(DTU::send(0, msg_ptr, msg_size_in * sizeof(DATA), 0x1111, 2), Error::NONE);
        ...
        for(size_t i = 0; i < msg_size_in; ++i)
            ASSERT_EQ(msg_ctrl[i], msg_ptr[i]);

        // we need the reply to get our credits back
        ASSERT_EQ(DTU::send(0, msg_ptr, msg_size_in * sizeof(DATA), 0, 2), Error::MISS_CREDITS);

        // send reply
        ASSERT_EQ(DTU::reply(1, reply_ptr, reply_size_in * sizeof(DATA), rmsg), Error::NONE);
        ...
        for(size_t i = 0; i < reply_size_in; ++i)
            ASSERT_EQ(msg_ctrl[i], reply_ptr[i]);
        // free slot
        ASSERT_EQ(DTU::ack_msg(2, rmsg), Error::NONE);
    }
}
