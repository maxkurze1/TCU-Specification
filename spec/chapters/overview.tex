...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    \node[draw=black, fill=white,
          ...
          minimum width=3.9em, minimum height=1.8em] (pex) {PEMux};
...
complex core with caches and virtual memory support and a multiplexer (PEMux) that supports multiple
applications on the same PE. These features require TCU extensions for virtual memory (\extbox{vm})
and PE sharing (\extbox{pemux}). This specification highlights the portions that are only required
if an extension should be supported correspondingly.
...
\extstart{pemux} The \emph{privileged registers} are intended for privileged software running on PEs
that are shared among multiple VPEs. The privileged software is called \emph{PEMux} for PE
multiplexer. PEMux should make sure that unprivileged software running on the same PE cannot access
the privileged registers. \extend{}
...
\begin{tabular}{ p{3cm} | c | c | l }
  ...
  \hline
  \hline
  \texttt{0xF000\_1000} & \texttt{CORE\_REQ} & PrivRegs & TCU-core interactions \extstart{pemux} \\
  ...
  \texttt{0xF000\_1020} & \texttt{OLD\_VPE} & PrivRegs & Old VPE when changing VPEs \extend{} \\
\end{tabular}\\[1em]
