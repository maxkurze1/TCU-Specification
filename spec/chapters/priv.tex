...
\begin{algorithm}[H]
    ...
    \BlankLine
    $insert\_tlb\_entry(vpe, virt, phys, flags)$\;
    $PRIV\_CMD.op \gets 0$\;
...
\begin{algorithm}[H]
    \uIf{COMMAND.op != 0}{
      ...
      \uElseIf{abort\_local\_xfer(ABORT)}{
        $PRIV\_CMD.arg0 \gets 1$\;
      }
    }
    \BlankLine
...
the TCU issues the next core request, if there is any. New core requests, as described in
\rref{code:corereqstart}, are appended to the queue via \texttt{queue\_foreign\_msg\_req()} (see
\rref{sec:unprivcmdspseudo}) and \texttt{queue\_xlate\_req()} (see \rref{sec:xlates}) and started,
if possible.

\subsection{Pseudo Code}

The following pseudo code describes the behavior in more detail:

\begin{algorithm}[H]
    \SetKwFunction{FMain}{queue\_xlate\_req}
    \SetKwProg{Fn}{Function}{:}{}
    \Fn{\FMain{$vpe$, $virt$, $access$, $can\_pf$}}{
      $queue.enqueue(vpe\ |\ virt\ |\ access\ |\ can\_pf\ |\ 2)$\;
      $queue\_try\_issue()$\;
    }
    \textbf{End Function}
    \BlankLine
    ...
    \SetKwProg{Fn}{Function}{:}{}
    \Fn{\FMain{$vpe$, $ep$}}{
      $queue.enqueue(vpe\ |\ ep\ |\ 3)$\;
      $queue\_try\_issue()$\;
    }
    \textbf{End Function}
    \BlankLine

    \SetKwFunction{FMain}{queue\_try\_issue}
    \SetKwProg{Fn}{Function}{:}{}
    \Fn{\FMain{}}{
...
\begin{algorithm}[H]
    \uIf{CORE\_REQ.type == 1 and not queue.empty()}{
      $cur = queue.dequeue()$\;
      \uIf{cur.type == 2}{
        $flags \gets CORE\_REQ.flags$\;
        \uIf{flags == 0}{
          $abort\_local\_xfer(PAGEFAULT)$\;
        }
        \uElse{
          $insert\_tlb\_entry(CORE\_REQ.vpe, cur.virt, CORE\_REQ.phys, flags)$\;
          $resume\_local\_xfer()$\;
        }
      }
      $CORE\_REQ \gets 0$\;
    ...
    \caption{Dequeuing and finishing core requests.}
...
\begin{register}{H}{CORE\_REQ}{\texttt{0xF000\_2000}}
  ...
  \begin{regdesc}\begin{reglist}
    \item[type] the type (0 = idle, 1 = response, 2 = translation, 3 = foreign message)
  \end{reglist}\end{regdesc}
\end{register}
...
\begin{register}{H}{CORE\_REQ for translation responses}{\texttt{0xF000\_2000}}
  ...
  \regfieldb[gray!20]{reserved}{5}{7}%
  \regfieldb{flags}{5}{2}%
  \regfieldb{type}{2}{0}\regnewline%
  \begin{regdesc}\begin{reglist}
