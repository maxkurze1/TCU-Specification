...
\begin{algorithm}[H]
    ...
    \uIf{ep.type != MEMORY\colorbox{pemux}{ or ep.vpe != $CUR\_VPE.id$}}{unpriv\_stop(INV\_EP)}
    \uIf{ep.rw \& READ == 0}{unpriv\_stop(NO\_PERM)}
    \uIf{DATA.size == 0}{unpriv\_stop(NONE)}
    \uIf{DATA.size + $ARG_1$ > ep.size}{unpriv\_stop(INV\_ARGS)}
    \BlankLine
...
\begin{algorithm}[H]
    $ep \gets$ read\_ep(COMMAND.ep)\;
    \uIf{ep.type != SEND or ep.reply != 0\colorbox{pemux}{ or ep.vpe != $CUR\_VPE.id$}}{unpriv\_stop(INV\_EP)}
    \uIf{DATA.size + sizeof(header) > ($1 << ep.msg\_sz$)}{unpriv\_stop(INV\_ARGS)}
    \BlankLine
    \uIf{COMMAND.arg\_0 != 0xFFFF}{
      $rep \gets read\_ep(COMMAND.arg_0$)\;
      \uIf{rep.type != RECEIVE}{unpriv\_stop(INV\_EP)}
    ...
    \uIf{ep.cur\_crd != 0x3F}{
        \uIf{ep.cur\_crd == 0}{unpriv\_stop(MISS\_CREDITS)}
        ep.cur\_crd -= 1\;
...
\begin{algorithm}[H]
    ...
    \uIf{ep.type != RECEIVE or (ep.buffer \& 0x7) != 0}{send\_ack(INV\_EP) and drop message}
    \BlankLine
    $idx \gets$ find\_slot(ep.occupied, ep.wpos, ep.slots, 0)\;
    \uIf{idx == -1}{send\_ack(NO\_RING\_SPACE) and drop message}
...
\begin{algorithm}[H]
    $ep \gets$ read\_ep(COMMAND.ep)\;
    \uIf{ep.type != RECEIVE or ep.rpl\_eps == 0xFFFF\colorbox{pemux}{ or ep.vpe != $CUR\_VPE.id$}}{unpriv\_stop(INV\_EP)}
    \BlankLine
    $idx \gets COMMAND.arg_0 >> ep.slot\_size$\;
    $sep = read\_ep(ep.rpl\_eps + idx)$\;
    \uIf{sep.type != SEND or sep.reply == 0}{unpriv\_stop(INV\_ARGS)}
    \uIf{DATA.size + sizeof(header) > ($1 << sep.msg\_sz$)}{unpriv\_stop(INV\_ARGS)}
    \BlankLine
    $(err, payload) \gets read\_local(DATA.address, DATA.size)$\;
    \uIf{err != 0}{unpriv\_stop(err)}
    \BlankLine
    \uIf{sep.cur\_crd == 0}{unpriv\_stop(MISS\_CREDITS)}
    sep.cur\_crd -= 1\;
    $write\_ep(ep.rpl\_eps + idx, sep)$\;
    \BlankLine
...
\begin{algorithm}[H]
    $ep \gets$ read\_ep(COMMAND.ep)\;
    \uIf{ep.type != RECEIVE\colorbox{pemux}{ or ep.vpe != $CUR\_VPE.id$}}{unpriv\_stop(INV\_EP)}
