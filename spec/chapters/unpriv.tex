...
\begin{itemize}
  ...
  \item \texttt{lookup\_tlb(virt, perm) -> (Phys, Error)}:\extstart{vm}\\
...
\begin{algorithm}[H]
    ...
    \uIf{(DATA.address \& 0xFFF) != ((DATA.address + DATA.size - 1) \& 0xFFF)}{unpriv\_stop(PAGE\_BOUNDARY)}
    \BlankLine
    $phys \gets$ DATA.address\;
    \extstart{vm}
    $(phys, err) \gets$ lookup\_tlb(DATA.address, WRITE)\;
    \uIf{err != 0}{unpriv\_stop(TLB\_MISS)}
    \extend{}
    \BlankLine
