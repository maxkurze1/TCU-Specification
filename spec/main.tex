...
\begin{scaletikzpicturetowidth}{0.98\linewidth}
  ...
  \path[undef] (0,0) rectangle (10,3) node[midway] {};
  \path[reg] (10,0) rectangle (11,3) node[midway] {r};
  \path[reg] (11,0) rectangle (27,3) node[midway] {crd\_ep};
  ...
  \path[undef] (0,3) rectangle (40,6) node[midway] {};
  \path[reg] (40,3) rectangle (48,6) node[midway] {pe};
  \path[reg] (48,3) rectangle (64,6) node[midway] {ep};
...
\begin{description}
  ...
  \item[pe{[23:16]}:] the ID of the destination PE
  \item[ep{[15:0]}:] the ID of the receive EP
  \item[r{[53:53]}:] whether this is a reply EP
  \item[crd\_ep{[52:37]}:] for reply EPs: the send EP at sender-side to receive credits
...
\begin{scaletikzpicturetowidth}{0.98\linewidth}
  ...
  \path[undef] (0,0) rectangle (5,3) node[midway] {};
  \path[reg] (5,0) rectangle (11,3) node[midway] {rpos};
  \path[reg] (11,0) rectangle (17,3) node[midway] {wpos};
  \path[reg] (17,0) rectangle (23,3) node[midway] {slot\_size};
  \path[reg] (23,0) rectangle (29,3) node[midway] {slots};
  \path[reg] (29,0) rectangle (45,3) node[midway] {rpl\_eps};
  \path[undef] (45,0) rectangle (61,3) node[midway] {};
  \path[reg] (61,0) rectangle (64,3) node[midway] {T};
...
\begin{description}
  ...
  \item[rpos{[58:53]}:] the read position (for message fetches) within the receive buffer
  \item[wpos{[52:47]}:] the write position (for message receptions) within the receive buffer
  \item[slot\_size{[46:41]}:] the size of one slot as a power of 2
  \item[slots{[40:35]}:] the number of slots in the receive buffer as a power of 2
  \item[rpl\_eps{[34:19]}:] the offset of the reply EPs
\end{description}
...
\begin{tabular}{ p{2cm} c }
  ...
  \begin{scaletikzpicturetowidth}{0.8\linewidth}
    ...
    \path[reg] (7,0) rectangle (39,3) node[midway] {arg$_0$};
    \path[reg] (39,0) rectangle (43,3) node[midway] {err};
    \path[undef] (43,0) rectangle (44,3) node[midway] {};
    \path[reg] (44,0) rectangle (60,3) node[midway] {ep};
  ...
  \begin{scaletikzpicturetowidth}{0.8\linewidth}
    ...
    \path[reg] (0,0) rectangle (32,3) node[midway] {size};
    \path[reg] (32,0) rectangle (64,3) node[midway] {address};
...
\begin{algorithm}[H]
    ...
    \uIf{ep.base\_addr + OFFSET > ep.size}{exit(INV\_ARGS)}
    \BlankLine
    $data \gets read\_remote(ep.PE, DATA.size, ep.base\_addr + OFFSET)$\;
...
\begin{algorithm}[H]
    ...
    $write\_remote(data, ep.PE, ep.base\_addr + OFFSET)$\;
    \BlankLine
    $COMMAND \gets 0$\;
...
\begin{tabular}{ p{2cm} c }
  ...
  \begin{scaletikzpicturetowidth}{0.8\linewidth}
    \begin{tikzpicture}[
      baseline=5ex,scale=\tikzscale,
      reg/.style={fill=gray!20,draw=black},
      undef/.style={fill=gray!20,draw=black,pattern=north east lines, pattern color=black}
    ]
    \path[reg] (0,0) rectangle (16,3) node[midway] {length};
    \path[reg] (16,0) rectangle (32,3) node[midway] {rep};
    \path[reg] (32,0) rectangle (48,3) node[midway] {sep};
    \path[reg] (48,0) rectangle (56,3) node[midway] {spe};
    \path[reg] (56,0) rectangle (62,3) node[midway] {rsize};
    \path[reg] (62,0) rectangle (64,3) node[midway] {fl};
    \path[reg] (32,3) rectangle (64,6) node[midway] {reply\_label};
    \path[reg] (0,3) rectangle (32,6) node[midway] {label};
...
\begin{description}
  \item[label{[63:32]}:] the label of the sender
  \item[reply\_label{[31:0]}:] the label the receiver should use for the reply
  \item[length{[63:48]}:] the payload size in bytes
  \item[rep{[47:32]}:] the receive endpoint ID for the reply at the sender side
  \item[sep{[31:16]}:] the sender endpoint ID
  \item[spe{[15:8]}:] the sender PE ID
  \item[rsize{[7:2]}:] the size of the reply message as a power of 2
  \item[fl{[1:0]}:] contains the following flags:
  \begin{itemize}
...
\begin{algorithm}[H]
    ...
    \uIf{(header.fl \& REPLY) == 0}{
      ...
      $\quad\quad\quad\ \  max\_msg\_size \gets header.rsize$\;
