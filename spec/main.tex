...
\begin{tabular}{ p{3cm} | c | c | l }
  ...
  \texttt{0xF000\_0030} & \texttt{COMMAND} & CmdRegs & Triggers internal commands \\
  \hline
  \texttt{0xF000\_0038} & \texttt{ABORT} & CmdRegs & Aborts internal commands \\
  \hline
  \texttt{0xF000\_0040} & \texttt{DATA} & CmdRegs & Specifies the data for commands \\
  \hline
  \texttt{0xF000\_0048} & \texttt{ARG$_1$} & CmdRegs & Additional argument for commands \\
  \hline
  \texttt{0xF000\_0050} & \texttt{EP$_{00}$} & EpRegs & First register of EP$_0$ \\
  \texttt{0xF000\_0058} & \texttt{EP$_{01}$} & EpRegs & Second register of EP$_0$ \\
  \texttt{0xF000\_0060} & \texttt{EP$_{02}$} & EpRegs & Third register of EP$_0$ \\
  \hline
  \texttt{0xF000\_0068} & \texttt{EP$_{10}$} & EpRegs & First register of EP$_1$ \\
  \texttt{0xF000\_0070} & \texttt{EP$_{11}$} & EpRegs & Second register of EP$_1$ \\
  \texttt{0xF000\_0078} & \texttt{EP$_{12}$} & EpRegs & Third register of EP$_1$ \\
  \hline
...
\begin{algorithm}[H]
    ...
    \uIf{ep.addr + $ARG_1$ > ep.size}{exit(INV\_ARGS)}
    \BlankLine
...
\begin{register}{H}{Message Header}{}
  ...
  \regfieldb{reply\_label}{32}{0}\regnewline%
  ...
  \begin{regdesc}\begin{reglist}
    ...
    \item[reply\_label] the label the receiver should use for the reply
...
\begin{algorithm}[H]
    ...
    $write\_ep(rep, ep)$\;
    \BlankLine
    \uIf{(header.flags \& REPLY) == 0 && header.rep != 0xFFFF}{
