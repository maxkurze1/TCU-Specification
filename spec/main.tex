...
\begin{itemize}
  ...
  stop the execution of the internal/external command. In both cases, the opcode is set to 0 and the error code to the given error, either using \texttt{COMMAND} (for internal commands) or \texttt{EXT\_CMD} (for external commands).
\end{itemize}
...
\begin{algorithm}[H]
    ...
    \uIf{ep.type != RECEIVE or (ep.buffer \& 0x7) != 0}{exit(send\_ack(INV\_EP) and drop message)}
    \BlankLine
...
from a remote PE. External commands are intended for the \myos{} kernel and are triggered by writing
to the \texttt{EXT\_CMD} register and feedback is given via this register as well. The
...
\begin{algorithm}[H]
    ...
    \uIf{force == 0 and ep.type == SEND}{
      \uIf{ep.cur\_crd != ep.max\_crd}{exit(MISS\_CREDITS)}
    }
