...
\begin{itemize}
  ...
  \item \texttt{find\_slot(mask, pos, slots) -> idx}:\\
  searches for an unset bit in the given mask from bit 0 to bit $(1 << slots) - 1$, starting at
  \texttt{pos}. The function returns the position of the unset bit or $-1$ if all bits are set.
...
\begin{algorithm}[H]
    ...
    \uIf{ep.cur\_crd != UNLIMITED}{
      \uIf{ep.cur\_crd < ep.max\_msg\_size}{exit(NO\_CREDITS)}
...
\begin{algorithm}[H]
    ...
    \BlankLine
    $idx \gets$ find\_slot(ep.occupied, ep.wpos, ep.slots)\;
...
\begin{algorithm}[H]
    ...
    \uIf{hd.flags \& REPLY\_ENABLED == 0}{exit(INV\_ARGS)}
    \BlankLine
    $hd.flags \gets 0$\;
...
\begin{algorithm}[H]
    ...
    \BlankLine
    $idx \gets$ find\_slot(ep.unread, ep.rpos, ep.slots)\;
